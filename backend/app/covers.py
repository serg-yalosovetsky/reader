...
# Порог формы кадра: og:image соцсетей — альбомный баннер (~1200×630, ratio 1.90),
# а обложка книги обычно портретная. Но «обычно» — не «всегда»: у техкниг и
# разборов обложка бывает альбомной, и порог 1.15 резал их вместе с баннерами.
#
# Замер по живой библиотеке (1454 книги, serg/tasks#997): среди забракованных
# только по форме кадра две группы с чистым разрывом между ними —
#   баннеры  122×41 (2.98), 160×83 (1.93), 1200×630 (1.90), 1404×517 (2.72);
#   обложки  1305×921 (1.42), 1650×1115 (1.48), 1200×750 (1.60), 737×500 (1.47).
# Порог стоит в этом разрыве. Двигать его — только по живым данным: он общий для
# ЗАПИСИ обложки (services._is_placeholder_cover) и её ПОКАЗА
# (reader._usable_cover), и расхождение этих двух суждений уже оставляло книгу
# без картинки без единой ошибки (serg/tasks#984).
_BANNER_MIN_ASPECT = 1.75
